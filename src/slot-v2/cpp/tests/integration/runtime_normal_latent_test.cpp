...
bool testOmenAndCZ() {
    ...
    }
    {
        ...
        ok &= runtime::armDebugFlag(s, debug::Channel::Presentation,
            static_cast<uint32_t>(normal_latent::Route::Direct));
        ok &= runtime::armDebugFlag(s, debug::Channel::NormalFlow,
...
bool testRouteDistribution() {
    ...
    int direct = 0, omen = 0, scripted = 0;
    for (int i = 0; i < 100000; ++i) {
        ...
        if (result.route == normal_latent::Route::Direct) ++direct;
        ...
        if (result.route != normal_latent::Route::Direct
            && (result.omen_games < 10u || result.omen_games > 25u
                || gate.active || !normal_latent::active(latent))) {
            return false;
        }
    }
    std::cout << "presentation: direct=" << direct
        << " omen=" << omen << " omen+CZ=" << scripted << '\n';
    return std::abs(direct - 1000) < 220
        && std::abs(scripted - 19800) < 1100
        && omen + direct + scripted == 100000;
}
}
int main() {
