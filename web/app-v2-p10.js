(() => {
  ...
  const TRANSLATE = {
    ...
    "RoleFlag::Bell9":"9枚ベル（中段有効ライン）","RoleFlag::Bell15":"15枚ベル（右上がり）","RoleFlag::ThreeMedal":"3枚役（右下がり）",
  ...
  function refresh(note) {
    ...
    $("debugState").textContent=JSON.stringify({
      build:"C++ 95/4/1 WASM V2 / STOPCTRL P10",setting:e.slot_v2_setting(),
  ...
  async function main(){
    ...
    try{
      ...
      const response=await fetch("slot-v2-p10.wasm?v=20260924-p10",{cache:"no-store"});
